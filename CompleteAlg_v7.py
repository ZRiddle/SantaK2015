...
def diffuse(Santa, tripid, w_limit = 1000, east = True, w_long = 50):
    '''
    Diffuse Presents east or west until weight cap is satisfied
    '''
    
    if east:
        dtrips = np.arange(tripid+1,min(tripid+4,Santa.trip_metrics.shape[0]))
    else:
        dtrips = np.arange(max(0,tripid-3),tripid)
...
def destroy_trip(Santa,tripid):
    ...
    dtrips = np.arange(max(tripid-2,0),min(tripid+3,Santa.trip_metrics.shape[0]))
...
'''
mask = Santa.trip_metrics.Ct < 10
for tripid in mask[mask].index:
...
# Destroy Small trips
tripid = 5
while Santa.trip_metrics.shape[0] > 1525:    
    stwrw = Santa.wrw
    if Santa.trip_metrics.ix[tripid].Weight < 700:
        Santa = destroy_trip(Santa,tripid)
    
        print 'Trip %.0f Destroyed!\n  New trip Count = %.0f\n  %.0f - New WRW'%(tripid,Santa.trip_metrics.shape[0],Santa.wrw)
        print '  Max Weight Trip = %.0f'%(Santa.trip_metrics.Weight.max())
        print '  Total trips over 1000 =',sum(Santa.trip_metrics.Weight > 1000)
    
    tripid += 23
    if tripid >= Santa.trip_metrics.shape[0]:
        tripid = tripid - Santa.trip_metrics.shape[0] + np.random.randint(0,10)

'''


#Santa = initialize_trips_better(Santa)
#Santa.save_data('_v7_5')
...
while w_limit >= 1000:
    ...
    for tripid in trindex:
        if Santa.T[tripid].shape[0] > 1:
            ...
            # Diffuse
            swrw = Santa.wrw
            Santa = diffuse(Santa, tripid = tripid, w_limit = w_limit, east = east, w_long = w_limit-1000)
            gain = swrw - Santa.wrw
            if gain:
                print '%.0f - Diffusion Gain, Trip %.0f'%(gain,tripid)
            if Santa.T[tripid].shape[0] > 1:
                Santa = optimize_trip(Santa,tripid)     
            print '\n'
    ...
    '''
    # Destroy Small trips
    while Santa.trip_metrics.Weight.min() < 200 or Santa.trip_metrics.Ct.min() < 8:
        stwrw = Santa.wrw
        tm = Santa.trip_metrics.copy()
        mask = np.logical_or(tm.Weight < 200, tm.Ct < 8)
        tripid = tm[mask].index[0]
        Santa = destroy_trip(Santa,tripid)
