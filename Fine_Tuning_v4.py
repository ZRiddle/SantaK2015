...
class Trips:
    ...
    def update_trip_metrics(self,tripid):
        ...
        SPF = 0
        if self.T[tripid].Latitude.mean() < -60:
            SPF = 1
    ...
    def optimal_trip_count(self,tr_list,r1=2,r2=10,r3=1):
        '''
        Inputs
        trip list
        Search Parameters
        ...
        #print 'Avg Weight Per Trip = %.0f'%(trip_metrics.ix[tr_list].Weight.sum()/n_best),'\n'
        return n_best    
    ...
    def load_balance(self,tripid,nearby_ct=3):
        '''
        ...
        SPmask = self.trip_metrics.SPF == self.trip_metrics.ix[tripid].SPF
        # Find nearest trips
        ...
        # Set search criterea
        r1 = max(1,int(np.floor(w_total/1000)))
        r2 = max(r1+20,int(r1*2))
        r3 = min(1,int((r2-r1)/20.))
        
        # Search for optimal trip count
        n_opt = self.optimal_trip_count(tr_list,r1=r1,r2=r2,r3=r3)
...
#((Santa.trip_metrics.ix[1218].AvgLong - Santa.trip_metrics.AvgLong)**2).sort_values().head(15)

tr_list1 = [741,736,740,734,739,737,760,738,759,742]
tr_list2 = [1218,1217,1220,1238,1219]
tr_list3 = [30,31,33,29,32,35,34]
#tr_list = [30,31,33,29,32,35,34]

wrw_start = trip_metrics.ix[tr_list].WRW.sum()
# 6 - 18212923
def optimal_trip_count(tr_list):
    wrw_best = 0
    n_best = 0
...
if 1:
    _ = optimal_trip_count(tr_list1)
    _ = optimal_trip_count(tr_list2)
    _ = optimal_trip_count(tr_list3)
...
tr_list = (trip_metrics[trip_metrics.SPF == 1]).index.tolist()


# Southpole optimal
wrw_best = 0
n_best = 0
for n_trips in range(210,230,1):
    g1 = split_trips(tr_list,n_trips)
    wrw_new = 0
    for g in g1:
        wrw_new += wrw_trip(g)
