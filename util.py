...
# WRW
def WRW(all_trips,show_warning=True):
    
    #all_trips = sub.join(gifts)
    if 'TripId' not in all_trips.columns:
        all_trips['TripId'] = 0
    uniq_trips = all_trips.TripId.unique()
    
    
    if show_warning and any(all_trips.groupby('TripId').Weight.sum() > weight_limit):
