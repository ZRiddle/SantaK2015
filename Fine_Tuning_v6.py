...
class Trips:
    ...
    def swap_worst_trip(self,ntry=10,plot_it = True,greedOfTheNoob=True,SPF = True,verbose=1,w_max=1000):
        start_wrw = self.wrw
        stime = time.time()
        for k in range(ntry):
            pp= '~'*33+'\nLoop #'+str(k)
...
for ppp in range(5):
    ...
    for q in range(15):
        ...
        # Choose a random
        nearby_ct = np.random.randint(6,20)
        # Find Optimal
...
#Santa.diffuse_east(SP=0,verbose=True)
