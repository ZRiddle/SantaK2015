...
def optimal_swapping(Santa):
    trlist = Santa.trip_metrics.index
    # Choose trip j
    for j in trlist:
        start_wrw = Santa.wrw
        # Copy Trip
...
def optimal_split(trip):
    start_wrw = wrw_trip(trip)
    Lats = np.array(trip.Latitude.sort_values(ascending=False))
    LatSort = np.array(-trip.Latitude).argsort()
    mask = trip.Latitude > Lats[1]
    gains = np.zeros(trip.shape[0]-1) + start_wrw
    
    i = 0
    while mask.sum() < mask.shape[0]:
        g1 = trip[mask].copy()
        g2 = trip[np.logical_not(mask)].copy()
        
        g1.iloc[0,4] = g1.iloc[0,3]
        g2.iloc[0,4] = g2.iloc[0,3]
        
        if g1.shape[0] > 1:
            g1.iloc[1:,4] = haversine_vect(g1.iloc[1:],g1.iloc[:-1])
        if g2.shape[0] > 1:
            g2.iloc[1:,4] = haversine_vect(g2.iloc[1:],g2.iloc[:-1])
        
        gains[i] -= wrw_trip(g1) + wrw_trip(g2)
        
        i += 1
        mask.iloc[LatSort[i]] = True
    ...
    if gains.max() < 0:
        ...
        # Do optimal split
        opt_i = gains.argmax()
        mask.iloc[LatSort[:opt_i+1]] = True
        mask.iloc[LatSort[opt_i+1:]] = False
...
for tripid in Santa.trip_metrics.index:
    if not tripid % 25:
        print 'Testing TripId',tripid
        print 'Runtime = %.1f seconds'%(time.time() - st_time)
    gain, g1, g2 = optimal_split(Santa.T[tripid].copy())
    if gain > 0:
        print '%.0f - Gain for TripId %.0f'%(gain,tripid)
        print 'New Trip Lengths = (%.0f,%.0f)'%(g1.shape[0],g2.shape[0])
    ...
    for tripid in trindex:
        if Santa.T[tripid].shape[0] > 1:
            ...
            Santa = diffuse(Santa, tripid = tripid, w_limit = w_limit, 
                            east = east, w_long = 50, nearby = 8-i)
            gain = swrw - Santa.wrw
            totalgain += gain
