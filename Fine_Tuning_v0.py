...
class Trips:
    ...
    def __init__(self,T=None,trip_metrics=None,gifts=None):
        # Set initial tables
        if T == None:
            with open('trip_list_v1.pkl','r') as f:
    ...
    def destroy_trip(self,tripid):
        ...
        for g in self.T[tripid].index:
            loose_gifts.append(self.remove_gift(tripid,g,update=False))
        # Update trip metrics to 0s
        self.trip_metrics.ix[tripid] = [0,0,0,0,999,0]
        return loose_gifts
    ...
    def diffuse_gifts(self,loose_gifts):
        ...
        start_trip = self.trip_metrics.sort_values('Weight',ascending=False).index[0]
        curr_trip = start_trip
        # Create Eastward Mask
        east_trip_mask = self.trip_metrics.AvgLong >= self.trip_metrics.ix[start_trip].AvgLong
        eastmost_trip = self.trip_metrics[self.trip_metrics.SPF == SP].sort_values('Longitude',ascending=False).index[0]
        # Keep going east until weight satisfied
        while self.trip_metrics[east_trip_mask].Weight.max() > 1000 and curr_trip != eastmost_trip:
            # Keep removing presents until weight satisfied
            while self.trip_metrics[curr_trip].Weight > 1000:
                # Take east-most gift
            ...
            mask = np.logical_and(east_trip_mask,self.trip_metrics.Weight > 1000)
            if mask.sum():
                curr_trip = self.trip_metrics[mask].sort_values('Longitude').index[0]
        
        # Step 3 - Diffuse west
        curr_trip = start_trip
        # Create Eastward Mask
        east_trip_mask = self.trip_metrics.AvgLong <= self.trip_metrics.ix[start_trip].AvgLong
        eastmost_trip = self.trip_metrics[self.trip_metrics.SPF == SP].sort_values('Longitude',ascending=True).index[0]
        # Keep going east until weight satisfied
        while self.trip_metrics[east_trip_mask].Weight.max() > 1000 and curr_trip != eastmost_trip:
            ...
            mask = np.logical_and(east_trip_mask,self.trip_metrics.Weight > 1000)
            if mask.sum():
                curr_trip = self.trip_metrics[mask].sort_values('Longitude',ascending = False).index[0]
...
def swap_worst_trip(Santa,ntry=10,plot_it = True,greedOfTheNoob=True,SPF = True,verbose=1):
    ...
    for k in range(ntry):
        ...
        p = 1/(closest_trip.head(5)+.02)
        p /= p.sum()
        new_trip = np.random.choice(closest_trip.head(5).index,p=p)
...
'''
Algorithm - 
1. Choose a random trip weighted by "Badness"
2. Choose a random present weighted by "Badness"
3. Swap present to a close trip weighted by distance
'''


# Look for clusters of Longs with light weights

# Reverse of that, check a trip to split into 2 trips, only need to cluster on long. Very easy.

Santa.write_sub('SantaSub3.csv')




tr_list = [276,274,164]#,275]
#Santa.plot_trips(tr_list,title='1 Cluster')
...
plt.figure('Cluser at -60',figsize=(12,8))
plt.scatter(Santa.gifts.Longitude,gifts.Latitude,color = 'gray')
c=0
coords = [360,0,180,0]
for tripid in tr_list:
    #range(510,530):#
    Santa.plot_trip(tripid,newplot=False,color=colors[c])
    c += 1
    # Get boundries
    if Santa.T[tripid].Longitude.min() + 180 < coords[0]:
        coords[0] = Santa.T[tripid].Longitude.min() + 180
    if Santa.T[tripid].Longitude.max() + 180 > coords[1]:
        coords[1] = Santa.T[tripid].Longitude.max() + 180
    if Santa.T[tripid].Latitude.min() + 90 < coords[2]:
        coords[2] = Santa.T[tripid].Latitude.min() + 90
    if Santa.T[tripid].Latitude.max() + 90 > coords[3]:
        coords[3] = Santa.T[tripid].Latitude.max() + 90

band = .1
plt.xlim(coords[0]*(1-band)-180,coords[1]*(1+band)-180)
plt.xlim(-61,-63)
plt.ylim(coords[2]*(1-band)-90,coords[3]*(1+band)-90)
plt.title('Cluser at -60',fontsize=20)
plt.tight_layout()
...
'''
Destroy Trip j
for all 'free' presents:
    Decide whether j-1 is closer or j+1 is closer (NN)
    Add to that trip

Diffuse East
trip k = j+1

while weight_{trip k} > 1000:
    Take eastmost present
    push to trip k+1
...
#################################
# Testing WRW
mask = gifts.Longitude > 92
mask = np.logical_and(mask,gifts.Longitude<92.6)
mask = np.logical_and(mask,gifts.Latitude > -60)
...
g_temp.drop(['Cluster','TripId'],axis=1,inplace=True)
...
WRW_start = (g_temp.Distance.cumsum()*g_temp.Weight).sum()
WRW_start += (g_temp.Distance.sum()+g_temp.iloc[-1].LB)*10
...
#plt.scatter(g_temp['Longitude'], g_temp['Latitude'], s=g_temp['Weight'], color='red')
#plt.plot(g_temp['Longitude'],g_temp['Latitude'], color=colors[1])
...
print 'WRW trip 1 =',WRW1

WRW2 = (g2.Distance.cumsum()*g2.Weight).sum()
WRW2 += (g2.Distance.sum()+g2.iloc[-1].LB)*10
print 'WRW trip 2 =',WRW2
...
# Sort by Weight, cumulative sum up to 1000, find index
w_mask = g_temp['Weight'].sort_values().cumsum()<1000

g_little = g_temp[w_mask].copy()
g_big = g_temp[np.logical_not(w_mask)].copy()

g_little.sort_values('Latitude',ascending=False,inplace=True)
d = haversine_vect(g_little.iloc[:-1],g_little.iloc[1:])
g_little.iloc[1:,4] = d
g_little.iloc[0,4] = g_little.iloc[0,3]

g_big.sort_values('Latitude',ascending=False,inplace=True)
d = haversine_vect(g_big.iloc[:-1],g_big.iloc[1:])
g_big.iloc[1:,4] = d
g_big.iloc[0,4] = g_big.iloc[0,3]

WRW_little = (g_little.Distance.cumsum()*g_little.Weight).sum()
WRW_little += (g_little.Distance.sum()+g_little.iloc[-1].LB)*10
print 'WRW trip 1 =',WRW_little

WRW_big = (g_big.Distance.cumsum()*g_big.Weight).sum()
WRW_big += (g_big.Distance.sum()+g_big.iloc[-1].LB)*10
print 'WRW trip 2 =',WRW_big

print 'WRW little/big trips =',WRW_little+WRW_big
print 'WRW 2 Longitude trips =',WRW1+WRW2
...
# Where are the heavy presents?
plt.figure('Large Gifts',figsize=(17,11))
plt.scatter(gifts.Longitude,gifts.Latitude,color = 'gray')

mask50 = gifts.Weight>49
plt.scatter(gifts[mask50].Longitude,gifts[mask50].Latitude,color = 'red')
