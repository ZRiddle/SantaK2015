...
# Read in Data
# Save files as pkl
with open('trip_metrics_v5.pkl','r') as f:
    trip_metrics = pickle.load(f)
with open('trip_list_v5.pkl','r') as f:
    T = pickle.load(f)
...
class Trips:
    ...
    def load_balance(self,tripid,nearby_ct=3,verbose=False):
        '''
        ...
        min_n = int(np.floor(w_total/1000))
        if w_total/min_n > 1000:
            min_n += 1
            
        r1 = max(1,max(min_n,nearby_ct-2))
        r2 = max(r1+5,int(r1*1.5))
    ...
    def swap_worst_trip(self,ntry=10,plot_it = True,greedOfTheNoob=True,SPF = True,verbose=1):
        start_wrw = self.wrw
        stime = time.time()
        for k in range(ntry):
            pp= '~'*33+'\nLoop #'+str(k)
            ...
            if self.trip_metrics.ix[new_trip].Weight + self.T[worst_trip].ix[worst_present].Weight > 1000:
                if verbose:
                    print pp
                    print 'Too Heavy'
            else:
...
'''
for omg in range(1):
    Santa.swap_worst_trip(1000,
                          plot_it=False,
                          greedOfTheNoob=True,
                          SPF=True,
                          verbose=0)
                                
    Santa.save_data(path='_v5')
'''
...
for ppp in range(2):
    ...
    for q in range(70):
        ...
        # Choose a random
        nearby_ct = np.random.randint(3,12)
        # Find Optimal
        n_opt = Santa.load_balance(rand_trip,nearby_ct=nearby_ct,verbose=False)
        if n_opt != nearby_ct+1:
            print '\nLoop #'+str(q)
    ...
    for omg in range(3):
        Santa.swap_worst_trip(5000,
                              plot_it=False,
                              greedOfTheNoob=True,
                              SPF=True,
                              verbose=0)
                                    
    Santa.save_data(path='_v6')
'''


# No more trip remapping
for omg in range(30):
    Santa.swap_worst_trip(30000,
                          plot_it=False,
                          greedOfTheNoob=True,
                          SPF=False,
                          verbose=0)
                                    
    Santa.save_data(path='_v7')
